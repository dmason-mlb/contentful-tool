...
function sanitizeDescription(description) {
  ...
  // Remove any space-specific URLs that might contain space IDs
  return description.replace(/https:\/\/app\.contentful\.com\/spaces\/[a-zA-Z0-9]+/g, 'https://app.contentful.com/spaces/YOUR_SPACE_ID');
}
...
function generateQueryDocs(schema) {
  ...
  queryType.fields.forEach(field => {
    ...
    if (field.description) {
      docs += `${sanitizeDescription(field.description)}\n\n`;
    }
    ...
    if (field.args && field.args.length > 0) {
      ...
      field.args.forEach(arg => {
        ...
        if (arg.description) {
          docs += ` - ${sanitizeDescription(arg.description)}`;
        }
        if (arg.defaultValue) {
...
function generateObjectTypeDocs(schema) {
  ...
  objectTypes.forEach(type => {
    docs += `## ${type.name}\n\n`;
    if (type.description) {
      docs += `${sanitizeDescription(type.description)}\n\n`;
    }
  ...
  if (scalars.length > 0) {
    ...
    scalars.forEach(scalar => {
      ...
      if (scalar.description) {
        docs += `${sanitizeDescription(scalar.description)}\n\n`;
      }
    });
  }
  ...
  if (enums.length > 0) {
    ...
    enums.forEach(enumType => {
      ...
      if (enumType.description) {
        docs += `${sanitizeDescription(enumType.description)}\n\n`;
      }
      ...
      if (enumType.enumValues && enumType.enumValues.length > 0) {
        ...
        enumType.enumValues.forEach(value => {
          ...
          if (value.description) {
            docs += ` - ${sanitizeDescription(value.description)}`;
          }
  ...
  if (inputTypes.length > 0) {
    ...
    inputTypes.forEach(inputType => {
      ...
      if (inputType.description) {
        docs += `${sanitizeDescription(inputType.description)}\n\n`;
      }
  ...
  if (interfaces.length > 0) {
    ...
    interfaces.forEach(interfaceType => {
      ...
      if (interfaceType.description) {
        docs += `${sanitizeDescription(interfaceType.description)}\n\n`;
      }
...
async function generateDocumentation() {
  ...
  try {
    ...
    // Generate object type docs (sanitized)
    ...
    console.log('Clean documentation generated successfully!');
    console.log('Files created:');
    console.log('- docs/README.md (overview)');
    console.log('- docs/queries.md');
    console.log('- docs/mutations.md');
    console.log('- docs/object-types.md (sanitized)');
